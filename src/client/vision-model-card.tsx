...
// Official-card chrome, reproduced. The plugin-manager package does not export
// its form fields as values, and the purity gate forbids cross-plugin value
// imports anyway; the classes below copy the compiled official stylesheets
// (fields.module.css / card-form.module.css) under our own prefix. The rules
// are written entirely against the global `--dsw-alias-*` design tokens, so
// the form themes with light/dark exactly like the built-ins.
const CSS = `
.dvt-description{color:var(--dsw-alias-label-tertiary);font-size:13px;line-height:1.5;margin:0}
...
.dvt-checkRow+.dvt-field,.dvt-checkRow+.dvt-checkRow{border-top:.5px solid var(--dsw-alias-border-l2)}
...
/**
 * Configuration card for the vision namespace, rendered by the Plugin Manager
 * on the bundle's own page (`plugins.bundle.config`, keyed by the package
 * name; the page draws the title and crumb itself). `view: 'page'` gets the
 * staged form with its own save control — staged edits, one 保存 button,
 * official field styling. `view: 'summary'` is the one-liner variant. The
 * model picker lists the Host-generation model catalog (dsh's single source
 * of configured models); manual entry stays available as the fallback, and a
 * refresh re-reads the catalog. Saved edits reach later requests immediately
 * through the Host half's read-through config — no session restart.
 */
export function VisionModelCard(face: VisionCardFace & PluginConfigViewProps): ReactNode {
  const { scope, listModels, lastError, refresh, view } = face
  ...
  if (view === 'summary') {
    const line = value.enabled === false
      ...
      : selected
        ? `识图模型:${selected}`
        : '识图模型未配置'
    return h('p', { className: 'dvt-description' }, line)
  }
